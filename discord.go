...
ignore_check:
	...
	if len(messageEvent.Mentions) == 1 && messageEvent.Mentions[0].ID == botID && messageEvent.Content == fmt.Sprintf("<@%s>", botID.String()) {
		Log.Debugf("bot was pinged")
		ping, _ := getMentions("discord", botName, guildID, chanID)
		if err := sendDiscordMessage(botSession, channel, messageEvent.Author, ping.Response, botName); err != nil {
			Log.Error(err)
		}
		if err := sendDiscordReaction(botSession, channel, message, ping.Reaction); err != nil {
			Log.Error(err)
		}
		return
	}

	// Check for commands, regex, and keywords
	...
	if len(response) == 0 {
		for _, mentioned := range messageEvent.Mentions {
			if mentioned.ID == botID {
				...
				_, mention := getMentions("discord", botName, guildID, chanID)
